...
5. Quando Python, browser locale e ricezione degli eventi sono disponibili, creare un manifest 0.4 conforme a [references/visual-editor.md](references/visual-editor.md), inizialmente in `candidato_selezionato`, con composizioni autonome per `4x5`, `1x1` e `9x16`. Impostare `presentation.output_mode` sul formato attivo iniziale (il primo elenco in `formats`), non su `all`: l'utente lavora tipicamente su un solo rapporto e la consegna finale non deve includere gli altri due senza una scelta esplicita.
